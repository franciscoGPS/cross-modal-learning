...
def build_settings(config):
   ...
   settings = {
    'test_size':test_size[config[0]],
    'twin':twin[config[1]],
    'thop':thop[config[2]],
    'channels':channels[config[3]],
...
def get_configs(config):
   ...
   settings = {'learning_rate':learning_rate[config[0]],
    'reg_par':reg_par[config[1]],
    'outdim_size':outdim_size[config[2]],
    'neurons_lstm':neurons_lstm[config[3]],
    'neurons1':neurons1[config[4]],
    'neurons2':neurons2[config[5]],
    'neurons3':neurons3[config[6]],
    'dropout':dropout[config[7]],
    'validation_split':validation_split[config[8]],
    'activation_model1':activation_model1[config[9]],
    'activation_model2':activation_model2[config[10]],
    'activation_model3':activation_model3[config[11]],
    'dense_size':dense_size[config[12]],
    'activation_lstm': activation_lstm[config[13]]}
   
   return settings

def start_process(samples, datadirs, network, random_moods = 0,  label = "" , epoch_input=3):
   results = pd.DataFrame()

   
   datadirs = ["songs/synced/30s_cuts2"]
   ...
   for directory in range(len(datadirs)):
      overall_results = {'gtg': [], 'mfcc': [], 'dataset_config':[], 'dataset': []}
      ## After tests, the settings can be fixed to accept only one set of settings per test. 
      ...
      for setting in range(len(dataset_settings)):
         ...
         if not np.DataSource().exists(pickle_name):
            ...
            print("No suitable dataset found. I'll try to extrac it from: ")
            print( datadirs[directory])
            print("Mood to use are: ", MOODS)
            print("")
            print("")
            print("")
         ...
         try:
            if network == "CNN":
               values = np.expand_dims(x_gtgstd, axis=3)
...
def train_deepcca(X1, y1, X2, y2, network = 0, epochs=20, batch_size=20, test_size = 0.3, label="" ):
   ...
   num_classes = np.unique(y1).shape[0] 
   ...
   ##This step extract input shapes 
   if len(X1.shape) > 3:
   ...
   for config in range(len(configs)):
      ...
      try:
         ...
         if not os.path.isdir("diagrams"):
           create_sub_dir = "mkdir "+ "diagrams"
           subprocess.call(create_sub_dir, shell=True)
         plot_model(model, to_file='diagrams/'+network+'_moldel_'+label+'_'+date+'.png')

         if label == "":
          label = network
         model, hist = train_model(model, data1, data2, epoch_num, batch_size, label)
         ...
         #new_data = test_dcca(model, data1, data2, 1024, DCCA)

         
         #svm_classify(test_dcca(model, data1, data2, 1024, DCCA), C=0.01, axis=0, dual=False)

         
         # Training and testing of SVM with linear kernel on the view 1 with new features
         
         #[test_acc, valid_acc] = svm_classify(new_data, C=0.01, axis=0, dual=False)
         [test_acc, valid_acc] = svm_classify(test_dcca(model, data1, data2, 1024, DCCA), C=0.01, axis=0, dual=False)
         #ipdb.set_trace()
...
def plot_results(hist, label):
   ...
   plt.ylabel("Loss")
   plt.title(label + " train_loss vs val_loss")
   plt.grid(True)
   ...
   try:
      ...
      plt.xlabel("Num. Epochs")
      plt.ylabel("Loss")
      plt.title(label + "train_acc vs val_acc")
      plt.grid(True)
...
def main(argv):
   ...
   random_moods = 20
   try:
      opts, args = getopt.getopt(argv,"hi:se:l:n:m:e:d:",["ifile=", "samples="])
   ...
   for opt, arg in opts:
      if opt == '-h':
         print( 'keras_build.py -s <samples=-1>  -l <label=""> -n <network="GRU/DRNN/BLSTM"> -e <epochs=1> -d <datadirs="">')
         sys.exit()
      ...
      elif opt in ("-d", "--datadirs"):
         datadirs.append(str(arg.strip()))
         print(datadirs)

         
   start_process(samples, datadirs, network, random_moods, label=label, epoch_input=epochs)
