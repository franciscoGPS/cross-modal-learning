...
from DeepCCA.models import create_model
from DeepCCA.metrics import Metrics
import ipdb


def train_model(model, data1, data2, epoch_num, batch_size, label):
    """
    trains the model
    # Arguments
    ...
    filename = label +"_"+ "temp_weights.hdf5"
    ...
    hist = model.fit([train_set_x1, train_set_x2], train_set_y1, batch_size=batch_size, 
        epochs=epoch_num, shuffle=True, validation_data=([valid_set_x1, valid_set_x2],
         valid_set_y1),callbacks=[reduce_lr, early_stopping, checkpointer])

    model.load_weights(filename)

    results = model.evaluate([test_set_x1, test_set_x2], test_set_y1, batch_size=batch_size, verbose=1)

    print('loss on test data: ', results)

    results = model.evaluate([valid_set_x1, valid_set_x2], valid_set_y1, batch_size=batch_size, verbose=1)
    print('loss on validation data: ', results)
    return model, hist

def train_single_branch_model(model, data1, data2, epoch_num, batch_size, label):
    """
    trains the model
    # Arguments
        data1 and data2: the train, validation, and test data for view 1 and view 2 respectively. data should be packed
    ...
    early_stopping = callbacks.EarlyStopping(monitor='val_loss', patience=4, verbose=2, mode='auto')
    # tbCallBack = callbacks.TensorBoard(log_dir='./Graph', histogram_freq=0, write_graph=True, write_images=True)
    # best weights are saved in "temp_weights.hdf5" during training
    # it is done to return the best model based on the validation loss
    filename = label +"_"+ "temp_weights.hdf5"
    ...
    hist = model.fit(train_set_x1, train_set_y1, batch_size=batch_size, 
        epochs=epoch_num, shuffle=True, validation_data=(valid_set_x1,
         valid_set_y1),callbacks=[reduce_lr, early_stopping, checkpointer])
...
def test_model(model, data1, data2, outdim_size, apply_linear_cca):
    ...
    if data2 == None:
        for k in range(3):
            pred_out = model.predict(data1[k][0])
            new_data.append([pred_out, data1[k][1]])        
    else:
